function draw(){
	if(state < 1){
		...
		var pty = 0;
		var temp_key = 0;
		var tmtx = player_x + ptx;
		var tmty = player_y + player_y_vel;
		...
		do{
			...
			if(state < 1){
				...
				if(world_dynamic[i][7] != 0){
					...
					/* if player on moving platform, move player x (bugged?) */
					if(platform === i){
						ptx += world_dynamic[i][7]
					}
				...
				if(world_dynamic[i][10] != 0){
					...
					/* if player on moving platform, move player y (bugged?) */
					if(platform === i){
						pty += world_dynamic[i][10]
					}
			...
			if(ptx!=0||pty!=0||player_y_vel!=0||world_dynamic[i][7]!=0||world_dynamic[i][10]!=0){
				var tmpgx = world_dynamic[i][0] + world_dynamic[i][2];
				var tmpgy = world_dynamic[i][1] + world_dynamic[i][3];

				/* check if player position + movmenet is within bounds of object */
				if(!(tmtx-20>tmpgx||tmtx+20<world_dynamic[i][0]||tmty-20>tmpgy||tmty+20<world_dynamic[i][1])){
					...
					if(world_dynamic[i][4] === 1 || world_dynamic[i][4] === 's'){
						...
						if(player_y_vel != 0
						 ...
						 && player_x != tmpgx + 20){
							if(player_y_vel > 0){
								if(tmty <= world_dynamic[i][1]-10 && tmty > world_dynamic[i][1] - 20){
									can_jump = 1;
									player_y_vel = world_dynamic[i][1] - player_y - 20;
									pty = 0;
									tmty = player_y + player_y_vel;
									if(world_dynamic[i][7] != 0){
										ptx += world_dynamic[i][7]
									}
									platform = i
								}
							}else if(tmty < tmpgy + 20 && tmty >= tmpgy + 10){
								player_y_vel = tmpgy - player_y + 20;
								tmty = player_y + player_y_vel
							}
						}
						...
						if(platform != i){
							...
							if(key_right && !key_left
							 ...
							 && player_x != tmpgx + 20
							 && player_x < world_dynamic[i][0]){
								ptx = world_dynamic[i][0] - player_x - 20;
								tmtx = player_x + ptx
							}
						}
		...
		platform = -1;
		player_x += Math.round(ptx);
		player_y += Math.round(pty + player_y_vel);
...
function setmode(newmode, newgame){
	...
	/* main menu mode */
		...
		get('page').innerHTML = '<div style=display:inline-block;text-align:left;vertical-align:top><div class=c><a href=/><b>Platform-2D</b></a></div><hr><div class=c><a onclick=setmode(3,1)>Generate Random Level</a></div><hr><div class=c><a onclick=setmode(4,1)>A Pit of Your Design</a><br><a onclick=setmode(5,1)>Booster Towers</a><br><a onclick=setmode(6,1)>Keys of a Father</a><br><a onclick=setmode(7,1)>Tutorial Island</a><br><a onclick=setmode(8,1)>Village of the Wolves</a></div><hr><div class=c><label><input'
...
var player_y_vel = 0;
var settings=[
	ls.getItem('platform-0')===null?1:parseFloat(ls.getItem('platform-0')),
	ls.getItem('platform-1')===null,
	ls.getItem('platform-2')===null?-10:parseFloat(ls.getItem('platform-2')),
	ls.getItem('platform-3')===null?.5:parseFloat(ls.getItem('platform-3')),
	ls.getItem('platform-4')===null?9:parseFloat(ls.getItem('platform-4')),
	ls.getItem('platform-5')===null?25:parseInt(ls.getItem('platform-5')),
	ls.getItem('platform-6')===null?4:parseFloat(ls.getItem('platform-6')),
	ls.getItem('platform-7')===null,
	ls.getItem('platform-8')===null?'W':ls.getItem('platform-8'),
	ls.getItem('platform-9')===null?'AD':ls.getItem('platform-9'),
	ls.getItem('platform-10')===null?'H':ls.getItem('platform-10')
];
var state = 0;
var width = 0;
var world_dynamic = [];
